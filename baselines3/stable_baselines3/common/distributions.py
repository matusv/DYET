...
def make_proba_distribution(
    ...
    if isinstance(action_space, spaces.Box):
        ...
        return DictDistribution(action_space, dist_kwargs)
    else:
...
class DictDistribution(Distribution):
    ...
    def __init__(
        self, 
        dict_action_space: gym.spaces.Space, 
        use_sde: bool = False, 
        ...
        self.distributions = {}
        for key, space in dict_action_space.items():
            kwargs = dist_kwargs[key] if key in dist_kwargs else {}
            distribution = make_proba_distribution(space, use_sde, **kwargs)
            self.distributions[key] = distribution

    def proba_distribution_net(self, latent_dim: int, log_std_init=0.0) -> nn.Module:
        """
        Create the layer that represents the distribution:
        it will be the logits (flattened) of the MultiCategorical distribution.
        You can then get probabilities using a softmax on each sub-space.
        ...
        class DistributionNet(nn.Module):
            def __init__(self, distributions):
                ...
                for key, distribution in distributions.items():
                    ...
                    if isinstance(self.distributions[key], (CategoricalDistribution, MultiCategoricalDistribution)):
                        module = distribution.proba_distribution_net(latent_dim)
                    
                    elif isinstance(self.distributions[key], DiagGaussianDistribution):
                        module, log_std = distribution.proba_distribution_net(latent_dim, log_std_init)
                        self.log_stds[key] = log_std
        ...
        action_logits = DistributionNet(self.distributions) #nn.Linear(latent_dim, sum(self.action_dims))
        return action_logits, action_logits.log_stds
    ...
    def proba_distribution(
        ...
        #print("action_logits.size():", action_logits.size())

        begin = 0
        self.proba_distributions = {}
        for key, distribution in self.distributions.items():

            if isinstance(self.distributions[key], CategoricalDistribution):
                end = begin + distribution.action_dim
                proba_dist = distribution.proba_distribution(action_logits[:, begin:end])
                
            
            if isinstance(self.distributions[key], MultiCategoricalDistribution):
                end = begin + distribution.action_dims
                proba_dist = distribution.proba_distribution(action_logits[:, begin:end])
                ...
                #print("action_logits[begin:end]:", action_logits[:, begin:end])
                proba_dist = distribution.proba_distribution(action_logits[:, begin:end], log_std[key])
                
            #proba_dist = distribution.proba_distribution(action_logits[begin:end], log_std[key], latent_sde[key])
            self.proba_distributions[key] = proba_dist
            begin = end
    ...
    def log_prob(self, actions):
        # return th.cat(
        #     [dist.log_prob(action) for dist, action in zip(list(self.proba_distributions.values()), th.unbind(actions, dim=1))], dim=-1
        # )
        ...
        res = {}
        for i, k in enumerate(self.proba_distributions):
            res[k] = self.proba_distributions[k].log_prob(actions[k])
        
        return res
        #return th.tensor([r for k, r in res.items()])
        

    def entropy(self) -> th.Tensor:
        #return {k: d.entropy() for k, d in self.proba_distributions.items()}
        return th.stack([dist.entropy() for dist in self.proba_distributions.values()], dim=1).sum(dim=-1)

    def sample(self) -> th.Tensor:
        return {k: d.sample() for k, d in self.proba_distributions.items()}
        #return th.stack([dist.sample().squeeze() for dist in self.proba_distributions.values()], dim=-1)

    def mode(self) -> th.Tensor:
        return {k: d.mode() for k, d in self.proba_distributions.items()}
        #return th.stack([dist.mode().squeeze() for dist in self.proba_distributions.values()], dim=-1)

    def actions_from_params(self, action_logits: th.Tensor, log_std, latent_sde, deterministic: bool = False) -> th.Tensor:
        # Update the proba distribution
        #self.proba_distribution(action_logits)
        #return self.get_actions(deterministic=deterministic)
        ...
        begin = 0
        actions = []
        for key, distribution in self.proba_distributions.items():

            if isinstance(self.distributions[key], CategoricalDistribution):
                end = begin + distribution.action_dim
                distribution.proba_distribution(action_logits[:, begin:end])
            ...
            if isinstance(self.distributions[key], MultiCategoricalDistribution):
                ...
                distribution.proba_distribution(action_logits[:, begin:end], log_std[key])
            
            actions.append(distribution.get_actions(deterministic=deterministic))
            begin = end
        
        #print("actions:", actions)
        
        return th.stack([a.squeeze() for a in actions], dim=-1)

    def log_prob_from_params(self, action_logits: th.Tensor, log_std, latent_sde) -> Tuple[th.Tensor, th.Tensor]:
        actions = self.actions_from_params(action_logits, log_std, latent_sde)
        log_prob = self.log_prob(actions)
        return actions, log_prob
