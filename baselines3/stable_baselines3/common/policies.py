    ...
    def predict(
        ...
        if isinstance(actions, dict):
            for k, action in actions.items():
                action = action.cpu().numpy().reshape((-1,) + self.action_space[k].shape)
            actions = [actions,]
        else:
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        elif isinstance(self.action_dist, DictDistribution):
            self.action_net, self.log_std = self.action_dist.proba_distribution_net(
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        #print("latent_pi.size():", latent_pi.size())
        #print("latent_vf.size():", latent_vf.size())
        ...
        if isinstance(actions, dict):
            for k, action in actions.items():
                actions[k] = action.cpu().numpy().squeeze().reshape(-1,) #((-1,) + self.action_space[k].shape)
                #print(k, actions[k])
            actions = [actions,]
        else:
